            ...
            <div className="flex gap-3">
                ...
                <div className="flex-1 space-y-3">
                    ...
                    {selectedImages.length > 0 && (
                        <div className={`grid gap-2 rounded-2xl overflow-hidden border border-zinc-200 dark:border-zinc-800 ${selectedImages.length === 1 ? 'grid-cols-1' : 'grid-cols-2'
                            ...
                            {selectedImages.map((img, index) => (
                                <div
                                    key={index}
                                    className={`relative group ${selectedImages.length === 3 && index === 0 ? 'row-span-2' : ''
                                        }`}
                                >
                                    <Image
                                        src={img}
                                    ...
                                    <div className="absolute top-2 left-2 opacity-0 group-hover:opacity-100 transition-opacity">
                                        <button className="bg-zinc-900/70 hover:bg-zinc-900/80 text-white px-3 py-1.5 rounded-full text-xs font-bold transition-colors">
                                            Edit
                                        </button>
                                    </div>
                    ...
                    <div className="flex items-center justify-between pt-1">
                        <div className="flex items-center gap-1 relative">
                            <button
                                ...
                                className="p-2 rounded-full text-sky-500 hover:bg-sky-50 dark:hover:bg-sky-950/20 transition-colors cursor-pointer disabled:opacity-50 disabled:cursor-not-allowed"
                                title="Media"
                            >
                        ...
                        <div className="flex items-center gap-2">
                            ...
                            <button
                                ...
                                className="px-5 py-1.5 text-zinc-600 dark:text-zinc-400 font-bold hover:bg-zinc-50 dark:hover:bg-zinc-800 rounded-full text-sm transition-all ml-1 cursor-pointer"
                            >
                                Cancel
                            </button>
                            <button
                                disabled={!text.trim() && selectedImages.length === 0}
                                className="px-5 py-1.5 bg-zinc-900 dark:bg-zinc-100 text-white dark:text-zinc-900 font-bold rounded-full text-sm disabled:opacity-50 hover:bg-zinc-800 dark:hover:bg-zinc-200 transition-all ml-1 cursor-pointer"
                            >
                                Post
                            </button>
                        </div>
                    ...
                    {showEmojiPicker && (
                        <div
                            ref={emojiPickerRef}
                            className="mt-3 bg-white dark:bg-black border border-zinc-200 dark:border-zinc-800 rounded-2xl flex flex-col overflow-hidden animate-in fade-in slide-in-from-top-2 duration-300 w-full"
                        >
                            ...
                            <div
                                ...
                                className="h-[250px] overflow-y-auto p-3 scroll-smooth relative"
                            >
                                {searchQuery ? (
                                    <div className="grid grid-cols-8 sm:grid-cols-10 gap-1">
                                        {filteredEmojis?.map((emoji, i) => (
                                            <button
                                                key={i}
                                                onMouseEnter={() => setHoveredEmoji({ char: emoji, category: "Search" })}
                                                onMouseLeave={() => setHoveredEmoji(null)}
                                                onClick={() => onEmojiSelect(emoji)}
                                                className="w-8 h-8 flex items-center justify-center text-xl hover:bg-zinc-100 dark:hover:bg-zinc-800 rounded-lg transition-colors"
                                            >
                                                {emoji}
                                            </button>
                                        ))}
                                    ...
                                    <>
                                        {recentEmojis.length > 0 && (
                                            <div id="emoji-cat-Recent" className="mb-4">
                                                <h3 className="text-xs font-bold text-zinc-500 mb-2 uppercase tracking-tight">Recently Used</h3>
                                                <div className="grid grid-cols-8 sm:grid-cols-10 gap-1">
                                                    {recentEmojis.map((emoji, i) => (
                                                        <button
                                                            key={i}
                                                            onMouseEnter={() => setHoveredEmoji({ char: emoji, category: "Recent" })}
                                                            onMouseLeave={() => setHoveredEmoji(null)}
                                                            onClick={() => onEmojiSelect(emoji)}
                                                            className="w-8 h-8 flex items-center justify-center text-xl hover:bg-zinc-100 dark:hover:bg-zinc-800 rounded-lg transition-colors"
                                                        >
                                        ...
                                        {Object.entries(emojis).map(([category, list]) => (
                                            <div key={category} id={`emoji-cat-${category}`} className="mb-4">
                                                <h3 className="text-xs font-bold text-zinc-500 mb-2 uppercase tracking-tight">{category}</h3>
                                                <div className="grid grid-cols-8 sm:grid-cols-10 gap-1">
                                                    {list.map((emoji, i) => (
                                                        <button
                                                            key={i}
                                                            onMouseEnter={() => setHoveredEmoji({ char: emoji, category })}
                                                            onMouseLeave={() => setHoveredEmoji(null)}
                                                            onClick={() => onEmojiSelect(emoji)}
                                                            className="w-8 h-8 flex items-center justify-center text-xl hover:bg-zinc-100 dark:hover:bg-zinc-800 rounded-lg transition-colors"
                                                        >
                            ...
                            <div className="p-4 bg-white dark:bg-zinc-950 border-t border-zinc-100 dark:border-zinc-800 flex items-center justify-between">
                                <div className="flex items-center gap-3">
                                    <div className="w-12 h-12 flex items-center justify-center bg-zinc-50 dark:bg-zinc-900 rounded-full border border-zinc-100 dark:border-zinc-800 shadow-sm text-3xl transition-all">
                                        {hoveredEmoji?.char || "👋"}
                                    </div>
                                    <div className="flex flex-col">
                                        <span className="text-[15px] text-zinc-900 dark:text-zinc-100 font-bold">
                                            {hoveredEmoji ? "Emoji" : "Welcome"}
                                        </span>
                                        <span className="text-[13px] text-zinc-500 font-medium">
                                            {hoveredEmoji ? hoveredEmoji.category : "Choose an emoji"}
                                        </span>
                                    </div>
                                </div>
                                <button
                                    onClick={() => setShowEmojiPicker(false)}
                                    className="w-11 h-11 flex items-center justify-center bg-sky-500 text-white rounded-full shadow-lg hover:bg-sky-600 transition-all hover:scale-105 active:scale-95"
                                >
